package com.devyd.tastearticles.vm
...
import com.devyd.tastearticles.models.TasteArticleResult
...
@HiltViewModel
class ComposeTasteArticleListViewModel @Inject constructor(
    private val getCategoryWeightsUseCase: GetCategoryWeightsUseCase,
    private val getArticleUseCase: GetArticleUseCase
) :
    ViewModel() {
    private val _articles =
        MutableStateFlow<ComposeTasteArticleResult>(ComposeTasteArticleResult.Idle)
        ...
        viewModelScope.launch {
            ...
            if (isSwipeRefresh) {
                _articles.updateAndDelay {
                    ComposeTasteArticleResult.Refreshing
                }
            } else {
                _articles.update { ComposeTasteArticleResult.Loading }
            }
            ...
            if (getCategoryWeightsUseCaseResult is CategoryWeightUiStateResult.Failure) {
                _articles.update { ComposeTasteArticleResult.Failure(getCategoryWeightsUseCaseResult.error) }
                ...
                if (categoryWeightList.isEmpty()) {
                    ...
                    _articles.update { ComposeTasteArticleResult.NeedToCategorySetting }
                } else {
                    val articleUiStateList = mutableListOf<ArticleUiState>()
                    val min = categoryWeightList.minOf { it.weight }
                        .coerceAtLeast(Constants.CATEGORY_SLIDER_MIN.toInt())
                    ...
                    val result = ComposeTasteArticleResult.Success(
                        ArticlesUiState(
                            status = "status",
                            totalResults = 1,
